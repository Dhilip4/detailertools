...
import { AISCLibrary } from './components/AISCLibrary/AISCLibrary';
import ConnectionDetailing from './components/ConnectionDetailing/ConnectionDetailing';
import WeldingModule from './components/WeldingCopes/WeldingModule';
import { BLOG_POSTS } from './components/blogData';


// Navigation Types
type Page = 'home' | 'checker' | 'calculator' | 'blog' | 'aisc-library' | 'connection-detailing' | 'welding';
...
export default function App() {
  ...
  const navItems: { id: Page; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'aisc-library', label: 'AISC Library', icon: <Database className="w-4 h-4" /> },
    { id: 'connection-detailing', label: 'Connections', icon: <Database className="w-4 h-4" /> },
    { id: 'welding', label: 'Welding', icon: <Hammer className="w-4 h-4" /> },
  ...
  return (
    <div className="min-h-screen flex flex-col text-white">
            ...
            <div className="hidden md:flex items-center space-x-4">
              {navItems.map((item) => (
                <button
                  ...
                  className={`relative transition-all duration-300 font-medium hover:text-white ${currentPage === item.id ? 'text-[#76b900]' : 'text-gray-300'
                    }`}
                >
                  {item.label}
                  <span className={`absolute bottom-[-4px] left-0 h-[2px] bg-gradient-to-r from-[#76b900] to-[#8bc34a] transition-all duration-300 ${currentPage === item.id ? 'w-full' : 'w-0'
                    }`}></span>
                </button>
              ))}
            </div>
            ...
            <div className="flex flex-col space-y-4">
              {navItems.map((item) => (
                <button
                  ...
                  className={`flex items-center space-x-2 text-lg ${currentPage === item.id ? 'text-[#76b900]' : 'text-gray-300'
                    }`}
                >
                  {item.icon}
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 py-8 w-full">
        ...
        {currentPage === 'aisc-library' && <AISCLibrary />}
        {currentPage === 'connection-detailing' && <ConnectionDetailing />}
        {currentPage === 'welding' && <WeldingModule />}

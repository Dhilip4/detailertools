...
export default function Forum({ isAdmin = false, userEmail = '', showToast, addNotification, initialThreadId }: ForumProps) {
  ...
  const fetchThreads = async () => {
    ...
    // INSTANT LOAD FIX: If keys are missing, load demo immediately
    if (!isConfigured) {
    ...
    try {
        ...
        console.warn("Firebase unreachable. Loading Demo Mode.");
        setThreads(DEMO_THREADS);
    } finally {
        setIsLoading(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
      ...
      const newThreadData = {
          ...
          likedBy: [],
          time: currentDate,
          createdAt: isConfigured ? Timestamp.now() : new Date()
      };

      try {
          let docId = 'demo-' + Date.now();
          if (isConfigured) {
              const docRef = await addDoc(collection(db, "threads"), newThreadData);
              docId = docRef.id;
          }
          ...
          console.error(err);
          if(showToast) showToast("Error creating thread", "error");
      }
  };
